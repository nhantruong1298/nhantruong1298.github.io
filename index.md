---
layout: default
---

## 👨 About
  I'm Nhan Truong. I've been working as a Flutter mobile developer for three years. My duties included creating interfaces following designs, integrating APIs, and building and deploying products to the app store. I've always been interested in creating software, which is why I chose to follow this career path.

## 🔧 My Skill
  - Dart &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;⭐⭐⭐⭐⭐
  - Flutter &nbsp;&nbsp;&nbsp;⭐⭐⭐⭐⭐
  - Swift &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;⭐⭐⭐

## 📦 Projects
**KHMartin** <br>
  - Freelance job
  - Role: Flutter Mobile Dev _(Jul 2023 - Today)_
  - Flutter Mobile App for student records management
    - Create and edit transcripts for students.
    - Delegate management rights to members.
  - Tech:
    - <kbd>Flutter</kbd> <kbd>Firebase</kbd> <kbd>Supabase</kbd>
  - Sites: <br>
    - [Android](https://play.google.com/store/apps/details?id=com.martin_charity_community)

**TranSwap** <br>
  - Company TranSwap
  - Role: Flutter Mobile Dev _(Jul 2021 - Oct 2023)_
  - Flutter Mobile App for move money between countries
    - Simplifying Global Finance.
    - Make faster and cheaper payments around the world.
    - Convert to multiple foreign currencies and save more with highly competitive exchange rates.
  - Tech:
    - <kbd>Flutter</kbd> <kbd>Firebase</kbd> <kbd>App Center</kbd> <kbd>Azure</kbd>
  - Sites: <br>
    - [IOS](https://apps.apple.com/app/id1564013302), [Android](https://play.google.com/store/apps/details?id=com.transwap.luna)

**UEH Student** <br>
  - Company Aziworld
  - Role: Junior Flutter Mobile Dev _(Dec 2020 - May 2021)_
  - Flutter Mobile App for students
    - Check-in online.
    - UEH's latest Event announcement.
    - Look up and perform online services.
  - Tech:
    - <kbd>Flutter</kbd> <kbd>Firebase</kbd> 
  - Sites: <br>
    - [Android](https://play.google.com/store/apps/details?id=vn.edu.ueh.student&hl=vi&gl=US),[IOS](https://apps.apple.com/us/app/ueh-student/id1560073124)


## 📌 Volunteering

**MARTINO Charities**  _(Jun 2017 - Present)_<br>
  - Mobilizing and calling for donations for business trips in remote areas.
  - Participate in activities that promote charity.
  - Organize activities to visit lonely elderly people, orphanages, leprosy camps, and HIV camps.

**Club LIVECLUB - Open University**  _(Jan 2017 - Jan 2018)_<br>
  - Support club members in activities and answer their questions during the learning process. I also teach first-year students and beginners to learn programming, including how to learn and debug code.
  <br><br>

## 🧑‍🎓 Education
**Computer science**<br>
[Open University](https://ou.edu.vn/) - Ho Chi Minh, VietNam _(Dec 2016)_ <br>
